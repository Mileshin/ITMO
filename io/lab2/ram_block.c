...
static int rb_open(struct block_device *bdev, fmode_t mode) {
    ...
    printk(KERN_INFO "var2: Device is opened\n");
    printk(KERN_INFO "var2: Inode number is %d\n", unit);
...
static void  rb_close(struct gendisk *disk, fmode_t mode) {
    printk(KERN_INFO "var2: Device is closed\n");
    return ;
}
...
static int rb_transfer(struct request *req) {
    ...
    rq_for_each_segment(bv, req, iter) {
        ...
        if (bv.bv_len % RB_SECTOR_SIZE != 0) {
            printk(KERN_ERR "var2: Should never happen: "
        ...
        printk(KERN_DEBUG "var2: Sector Offset: %ld; Buffer: %p; Length: %d sectors\n",
    ...
    if (sector_offset != sector_cnt) {
        printk(KERN_ERR "var2: bio info doesn't match with the request info");
        ret = -EIO;
    }
...
static int __init rb_init(void) {
    ...
    /* Get Registered */
    rb_major = register_blkdev(rb_major, "var2");
    if (rb_major <= 0) {
        printk(KERN_ERR "var2: Unable to get Major Number\n");
        ramdevice_cleanup();
        return -EBUSY;
    }
    ...
    if (rb_dev.rb_queue == NULL) {
        printk(KERN_ERR "var2: blk_init_queue failure\n");
        unregister_blkdev(rb_major, "var2");
        ramdevice_cleanup();
        return -ENOMEM;
    }
    ...
    if (!rb_dev.rb_disk) {
        printk(KERN_ERR "var2: alloc_disk failure\n");
     ...
     */
    sprintf(rb_dev.rb_disk->disk_name, "var2");
    ...
    printk(KERN_INFO "var2: Ram Block driver initialised (%d sectors; %d bytes)\n",
